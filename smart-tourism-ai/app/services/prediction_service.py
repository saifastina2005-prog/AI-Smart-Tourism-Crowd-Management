...
MODEL_PATH = BASE_DIR / "ml_models" / "xgboost_model.pkl"
model = None

DEMO_LOCATIONS = [
    {"location_id": 1, "latitude": -37.8136, "longitude": 144.9631, "predicted_next_hour": 150, "capacity": 220, "utilization_ratio": 0.68, "risk_level": "Medium"},
    {"location_id": 2, "latitude": -37.8050, "longitude": 144.9610, "predicted_next_hour": 260, "capacity": 300, "utilization_ratio": 0.87, "risk_level": "High"},
    {"location_id": 3, "latitude": -37.8200, "longitude": 144.9550, "predicted_next_hour": 110, "capacity": 250, "utilization_ratio": 0.44, "risk_level": "Low"},
    {"location_id": 4, "latitude": -37.8165, "longitude": 144.9730, "predicted_next_hour": 280, "capacity": 320, "utilization_ratio": 0.88, "risk_level": "Critical"},
    {"location_id": 5, "latitude": -37.7995, "longitude": 144.9670, "predicted_next_hour": 120, "capacity": 190, "utilization_ratio": 0.63, "risk_level": "Medium"},
]
...
def predict_crowd(location_id: int):
    try:
        ...
        print(f"⚠ Prediction fallback used for location {location_id}: {exc}")
        demo = next((item for item in DEMO_LOCATIONS if item["location_id"] == location_id), DEMO_LOCATIONS[0])
        return {
            "location_id": location_id,
            "predicted_next_hour": float(demo["predicted_next_hour"]),
            "capacity": float(demo["capacity"]),
            "utilization_ratio": float(demo["utilization_ratio"]),
...
def multi_hour_forecast(location_id: int, hours: int = 24):
    try:
        ...
        print(f"⚠ Forecast fallback used for location {location_id}: {exc}")
        demo = next((item for item in DEMO_LOCATIONS if item["location_id"] == location_id), DEMO_LOCATIONS[0])
        base = float(demo["predicted_next_hour"])
        return [
            {"hour_ahead": step, "predicted_crowd": max(0, round(base + ((step - 1) * 8), 1))}
            for step in range(1, hours + 1)
        ]
...
def predict_all_locations():
    try:
        ...
        print(f"⚠ Falling back to demo map data: {exc}")
        return {
